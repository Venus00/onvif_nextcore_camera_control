...
exports.getVideoEncoderConfiguration = getVideoEncoderConfiguration;
exports.setVideoEncoderConfiguration = setVideoEncoderConfiguration;
exports.focusMove = focusMove;
exports.focusStop = focusStop;
const wsse_js_1 = require("./wsse.js");
const onvifClient_js_1 = require("./onvifClient.js");
const camera_1 = require("./camera");
const xml2js_1 = require("xml2js");
async function getVideoEncoderConfiguration(camId) {
    const cam = camera_1.cameras[camId];
    if (!cam)
        throw new Error("Camera not found");
    const securityHeader = (0, wsse_js_1.buildSecurityHeader)(cam.username, cam.password);
    const xml = `
  <s:Envelope xmlns:s="http://www.w3.org/2003/05/soap-envelope">
    ...
    const responseXml = await (0, onvifClient_js_1.sendSoapRequest)(cam.ip, cam.port || 80, xml, "http://www.onvif.org/ver10/media/wsdl/GetVideoEncoderConfiguration");
    // Parse SOAP response
    const json = await (0, xml2js_1.parseStringPromise)(responseXml || '', { explicitArray: false });
    try {
        const config = json["SOAP-ENV:Envelope"]["SOAP-ENV:Body"]["trt:GetVideoEncoderConfigurationResponse"]["trt:Configuration"];
        return {
            token: config.$.token,
            name: config["tt:Name"],
...
async function setVideoEncoderConfiguration(camId, config) {
    const cam = camera_1.cameras[camId];
    if (!cam)
        throw new Error("Camera not found");
...
`;
    try {
        console.log("to");
        const responseXml = await (0, onvifClient_js_1.sendSoapRequest)(cam.ip, cam.port || 80, xml, "http://www.onvif.org/ver10/media/wsdl/SetVideoEncoderConfiguration");
...
async function focusMove(camId, speed, token) {
    const cam = camera_1.cameras[camId];
    if (!cam)
        throw new Error("Camera not found");
    const securityHeader = (0, wsse_js_1.buildSecurityHeader)(cam.username, cam.password);
  ...
  <s:Envelope xmlns:s="http://www.w3.org/2003/05/soap-envelope">
    ...
    const responseXml = await (0, onvifClient_js_1.sendSoapRequest)(cam.ip, cam.port || 80, xml, "http://www.onvif.org/ver20/imaging/wsdl/Move");
    return responseXml;
}
async function focusStop(camId, token) {
    const cam = camera_1.cameras[camId];
    if (!cam)
        throw new Error("Camera not found");
    const securityHeader = (0, wsse_js_1.buildSecurityHeader)(cam.username, cam.password);
  ...
  <s:Envelope xmlns:s="http://www.w3.org/2003/05/soap-envelope">
    ...
    const responseXml = await (0, onvifClient_js_1.sendSoapRequest)(cam.ip, cam.port || 80, xml, "http://www.onvif.org/ver20/imaging/wsdl/Stop");
    return responseXml;
}
