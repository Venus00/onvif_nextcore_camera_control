...
export class PTZAPI {
  ...
  private parseResponse(text: string): ParsedConfig {
    ...
    for (const line of text.trim().split("\n")) {
      ...
      if (eqIndex > 0) {
        result[line.substring(0, eqIndex).trim()] = line
          .substring(eqIndex + 1)
          .trim();
      }
    }
  ...
  async setPtzProtocol(
    params: PTZProtocolParams,
    channel: number = 0
  ): Promise<string> {
    const parts: string[] = [];
    for (const [key, value] of Object.entries(params)) {
      if (key === "Attribute" && Array.isArray(value)) {
        value.forEach((v, i) =>
          parts.push(`Ptz[${channel}].Attribute[${i}]=${v}`)
        );
      } else {
        parts.push(
          `Ptz[${channel}].${key}=${encodeURIComponent(String(value))}`
        );
      }
    }
  ...
  async startTour(tourId: number, channel: PTZChannel = 0): Promise<string> {
    return this.ptzControl("start", channel, "StartTour", tourId, 0, 0);
  }

  async stopTour(tourId: number, channel: PTZChannel = 0): Promise<string> {
    return this.ptzControl("stop", channel, "StopTour", 0, 0, 0);
  }
  ...
  async setPresetConfig(
    ...
    const param = `PtzPreset[${channel}][${presetId}].Name=${Name}`;
    return await this.setConfig(param);
  }
  ...
  async setTourConfig(
    ...
    for (const [key, value] of Object.entries(params)) {
        value.forEach((preset, i) => {
          parts.push(
            `PtzTour[${channel}][${tourId}].Presets[${key}][${i}]=${preset}`
          );
        });
      }
  ...
  async setScanTourConfig(
    params: ScanTourParams,
    channel: number = 0,
    scanTourId: number = 0
  ): Promise<string> {
    const parts: string[] = [];
    for (const [key, value] of Object.entries(params)) {
      parts.push(
        `PtzScanTour[${channel}][${scanTourId}].${key}=${encodeURIComponent(String(value))}`
      );
    }
    return this.setConfig(parts.join("&"));
  }
  ...
  async setAutoPattern(
    recordState: boolean,
    channel: number = 0,
    patternId: number = 0
  ): Promise<string> {
    return this.setConfig(
      `AutoPattern[${channel}][${patternId}].RecordState=${recordState}`
    );
  }
  ...
  async setScheduledTask(
    ...
    for (const [key, value] of Object.entries(params)) {
      if (key === "AutoHomingTime") {
        parts.push(
          `PtzAutoMovement[${channel}][${taskId}].AutoHoming.Time=${value}`
        );
      } else {
  ...
  async setLimitPosition(
    channel: PTZChannel,
    type: 0 | 1 | 2 | 3 | 6 | 7 | 8
  ): Promise<string> {
    const url = this.buildUrl(
      `/cgi-bin/ptz.cgi?action=markLimit&channel=${channel}&type=${type}`
    );
    return (await this.request(url)).trim();
  }
